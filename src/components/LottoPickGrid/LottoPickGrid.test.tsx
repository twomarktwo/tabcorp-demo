...
describe("Test Lotto Pick Grid", () => {
  ...
  it('contains 35 lotto cells none selected', () => {
    ...
    for (let i = 0; i < 35; i++) {
      ...
      expect(cells[i].props.lottoNumber).toBe(checkCellNumber);
  ...
  it('contains 35 lotto cells some selected', () => {
    ...
    var appComponent = create(<LottoPickGrid startNumber={1} endNumber={35} cellsPerRow={10} pickedLottoNumbers={selectedValues} />);
    const rootInstance = appComponent.root;
    const cells = rootInstance.findAllByType(LottoCell, { deep: true });
    expect(cells.length).toBe(35);
    ...
    expect(selectedCells[0].props.lottoNumber).toBe(selectedValues[0]);
    expect(selectedCells[1].props.lottoNumber).toBe(selectedValues[1]);
    expect(selectedCells[2].props.lottoNumber).toBe(selectedValues[2]);
  });
});
